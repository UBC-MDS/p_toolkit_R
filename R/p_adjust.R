...
p_adjust <- function(data, pv_index, method, alpha=0.05){
  ...
  if(is.data.frame(data)){
    ...
    if(sapply(data[pv_index], is.numeric)){
      ...
      if(is.numeric(data)){
        ## convert matrix or verctor to data frame
  ...
  data <- data %>%
    ...
    mutate(
      ...
      bh_pvalue = p_value*rank/m
    ) %>%
    select(-rank)

  if(method == 'bh' | method == 'fdr'){
    return(data %>% select(p_value,bh_pvalue))
  }else if(method == 'bonf' | method == 'bonferroni'){
    return(data %>% select(p_value,bonf_pvalue))
  }else{
